...
run "check_dns_a_record" {
  ...
  variables {
    host = run.create_host.fqdn
  }

  assert {
    condition     = contains(data.dns_a_record_set.this.addrs, run.create_host.public_ip)
    error_message = "DNS record for ${run.create_host.fqdn} does not include ${run.create_host.public_ip} ip address"
  }
}
...
run "website_responded_200_with_correct_response_body" {
  ...
  variables {
    url = "${var.http_protocol}://${run.create_host.fqdn}"
  }
